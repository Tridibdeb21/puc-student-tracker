...
function getWeeklyWinner(results) {
    ...
    const candidates = Object.entries(weeklySolvesMap)
        .filter(([h, val]) => val.daysSolved >= 5);

    if (candidates.length === 0) return null;

    // Highest rating tiebreaker
    ...
    try {
        ...
        for (let handle of students) {
            ...
            for (let p of todayProblems) {
                ...
                else if (p.rating <= 1800) difficultyCount.medium++;
...
app.get("/api/contests/upcoming", async (req, res) => {
    try {
        ...
        let contests = cfRes.data.result
            .filter(c => c.phase === "BEFORE" || c.phase === "CODING") // upcoming or live
            .map(c => {
                const startTS = c.startTimeSeconds * 1000; // CF UTC timestamp in ms
                ...
                // Check if contest is soon (next 24 hours)
        ...
        // SORT by live first, then start time ascending
        contests.sort((a, b) => {
        ...
        // remove startTS before sending
        contests = contests.map(({startTS, ...rest}) => rest);
